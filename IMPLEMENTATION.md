...
- **#2 → #3 → #4 → #5 are strict-linear.** Each consumes the prior plan's contracts (state machine, dispatch, event bus, terminal bytes).
- **#6 → #7 → #8 are strict-linear** for the same reason on the frontend side.
- **#9 ships v0.1.0.** It depends on plans #1-#8 being done; it doesn't have to wait if there are emergency reasons but normally sequences last.
- **#10 is independent.** It can land any time after `design-tokens.json` is stable (which is now). Its Download CTA gracefully degrades to "Coming soon" until plan #9 ships at least one release.
...
- [ ] DESIGN.md, API.md, README.md, CLAUDE.md, TODOS.md, RELEASING.md are all current
...
1. Read `DESIGN.md`, `API.md`, `CLAUDE.md`, `TODOS.md` (in that order).
2. Read this file (`IMPLEMENTATION.md`).
3. Read `.claude/PRPs/plans/01-backend-skeleton.plan.md`. Pay attention to "Recommended PR split" and "Spike Acceptance" sections.
4. Open PR #1a (monorepo + tooling + CI). Land it.
5. Open PR #1b (Electron + IPC + tRPC). Land it.
6. Open PR #1c (DB + spike + contract test). Run the spike. If green, proceed to plan #2. If not, surface the failure, re-spec, do not paper over.
